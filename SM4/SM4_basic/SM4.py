#利用python实现sm4
from unittest import result


class SM4:
    def __init__(self,key:bytes):#key我们要使用字节的形式
        if not len(key) == 16:#16*8=128字节
            raise ValueError("SM4 key must be length of 16")
        self._key = self.Get_key(key)
        self.block_size = 16

    def encrypt(self,plaintext:bytes):
        return self.Root(plaintext,self._key)
    
    def decrypt(self,ciphertext:bytes):
        return self.Root(ciphertext,self._key[::-1])
    

    def Root(self,text:bytes,key_raw:list):
        _text = [0 for i in range(4)]
        for i in range(4):
            _text[i] = int.from_bytes(text[4*i:4*i+4],'big')
            #res = int.from_bytes(bytes,byteorder,signed = False)是将
            #bytes类型的变量x，转化成十进制整数，并存入res中，其中byteorder是大端还是小端存，而signed是是否考虑符号
        for i in range(32):
            box_in = _text[1]^_text[2]^_text[3]^key_raw[i];
            box_out = self._s_box(box_in)
            temp = _text[0]^box_out^self._rot_left(box_out,2)^self._rot_left(box_out,10)
            temp = temp^self._rot_left(box_out,18)^self._rot_left(box_out,24)
            _text = _text[1:] + [temp]
        _text = _text[::-1] #reverse the result
        #合并为128bit
        result = bytearray()
        for i in range(4):
            result.extend(_text[i].to_bytes(4,'big'))
        return bytes(result)

    def Get_key(self,key:bytes):
        key_row,key_temp = [0 for i in range(32)],[0 for i in range(4)]
        FK = [0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc]
        CK = [0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269, 0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
              0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249, 0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
              0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229, 0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
              0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209, 0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279]
        # 将 128bit 拆分成 4x32bit
        for i in range(4):
            temp = int.from_bytes(key[4*i:4*i+4],'big')
            key_temp[i] = temp^FK[i]
        for i in range(32):
            box_in = key_temp[1]^key_temp[2]^key_temp[3]^CK[i]
            box_out = self._s_box(box_in)
            key_row[i] = key_temp[0]^box_out^self._rot_left(box_out,13)^self._rot_left(box_out,23)
            key_temp = key_temp[1:] + [key_row[i]]
        return key_row
    
    @staticmethod #可不实例化类对象进行调用

    def _s_box(n:int):
        BOX = []
        result = bytearray()
        for item in list(n.to_bytes(4,'big')):
            result.append(BOX[item])
        return int.from_bytes(result,'big')

    @staticmethod
    def _rot_left(n,m):
        return ((n<<m)|(n>>(32-m))) & 0xFFFFFFFF



key = bytes.fromhex("0123456789ABCDEFFEDCBA9876543210")
plaintext = bytes.fromhex("0123456789abcdeffedcba9876543210")
sm4 = SM4(key)
print(sm4.encrypt(plaintext).hex())




